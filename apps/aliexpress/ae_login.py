...
class LoginAli(object):
    ...
    async def shop_list(self, shop_url, flag=True):
        ...
        # logger.info(f'url list========={len(url_list)}')
        if url_list:
            for i, url in enumerate(url_list):
                url = await self.page.evaluate('(url) => url.textContent', url)
                if 'https:' not in url:
                    url = f'https:{url}'
                    self.redis_conn.rpush('aliexpress_url1', url)
            try:
    ...
    async def check_login(self):
        ...
        with open('./apps/aliexpress/cookie.txt', 'w') as f:
            f.write(cookies)
